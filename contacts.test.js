...
// Configure axios to not follow redirects and throw on 3xx
axios.defaults.maxRedirects = 0;
axios.defaults.validateStatus = (status) => status < 300;  // Only accept 2xx
        ...
        [
          ...
          async () => {
            try {
              ...
              throw new Error(`It looks like your API contacts resource isn't properly setting the Content-Type header to "application/json". Make sure you are using res.json() method in your Express application to set the response type to JSON.`)
            }
          }
        ],
        [ 
      ...
      [
        [
          ...
          async () => {
            ...
            try {
              const response = await axios.post('http://localhost:8080/v1/contacts', data)
              expect(response.status).toBe(303)
              const location = response.headers.location || response.headers.Location || ''
              expect(location).toMatch(/^\/v1\/contacts\/\d+$/)
            } catch(error) {
              if (error.response) {
                expect(error.response.status).toBe(303)
